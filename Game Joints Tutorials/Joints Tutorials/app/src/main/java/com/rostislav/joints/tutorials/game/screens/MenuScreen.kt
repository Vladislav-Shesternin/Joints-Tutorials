...
import com.rostislav.joints.tutorials.game.box2d.bodiesGroup.BGMenu
import com.rostislav.joints.tutorials.game.box2d.bodiesGroup.BGYanYinTheme
...
class MenuScreen(override val game: LibGDXGame): AdvancedMouseScreen(game) {
    ...
    private fun createBG_Menu() {
        ...
        bgMenu.apply {
            ...
            bRegularBtnComment    .getActor()?.setOnTouchListener { navigateTo("EXIT") }
        }
    }
    ...
    private fun navigateTo(screenName: String) {
        if (screenName == "EXIT") stageUI.root.animHide(TIME_ANIM_SCREEN_ALPHA) { game.navigationManager.exit() }
        else stageUI.root.animHide(TIME_ANIM_SCREEN_ALPHA) { game.navigationManager.navigate(screenName, MenuScreen::class.java.name) }
    }
